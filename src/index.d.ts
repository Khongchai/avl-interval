...
export type ForEachCallback<Key, Value> = (
    node: Node<Key, Value>,
    index: number
) => void
export type TraverseCallback<Key, Value> = (
    node: Node<Key, Value>
) => void | boolean

export type Interval<Key> = {
    low: Node<Key, any> | null
    high: Node<Key, any> | null
}
...
export default class AVLTree<Key extends any, Value extends any> {
    ...
    findInterval(key: Key): Interval<Key> | null
    ...
    range(
        minKey: Key,
        maxKey: Key,
        visit: TraverseCallback<Key, Value>,
        context?: any
    ): AVLTree<Key, Value>
    load(
        keys: Array<Key>,
